...
/*
    PART 1 =>   select DISTINCT A.book_id from 
    ...
    select Distinct A.* from (select * from Books where price <= 400) A JOIN (select * from Books_Author) B on A.book_id = B.book_id JOIN (select DISTINCT book_id from Books_Genre where genre_id in ('GENR0001' , 'GENR0002' , 'GENR0003')) C on A.book_id = C.book_id ;
...
router.post("/books/select", async (req, res) => {
    ...
    let str_publ = "'" + publisher.join("' , '") + "'";
    let str_genre = "'" + genre.join("' , '") + "'";
    let str_author = "'" + author.join("' , '") + "'";
    ...
    let part1 = `select DISTINCT A.* from `;
