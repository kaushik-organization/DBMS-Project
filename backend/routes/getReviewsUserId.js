...
router.get("/reviews-UserId/:UserId",async(req,res)=>{
    try {
        ...
        const query = `select u.*,r.book_id,r.rating,r.comment from ((Reviews as r join Books as b on b.book_id = r.book_id) join User as u on r.user_id = u.user_id) where r.user_id = '${user_id}';`;
