...
router.post("/", async (req, res) => {
  ...
  const [books] = await conn.query(
    `select book_id , count as quantity from Basket_Books where basket_id = '${basket_id}'`
  );

  if (books.length == 0) return res.status(501).send("Not any book");

  for (let i = 0; i < books.length; i++) {
    let [[Available]] = await conn.query(
      `select quantity as Available from Sales where book_id = '${books[i].book_id}'`
    );
    if (!Available) return res.sendStatus(501);
    if (Available.Available < books[i].quantity)
      return res
        .status(209)
        .send({ book_id: books[i].title, Available: Available.Available });
  }

  const user_id = basket_id.replace("BASK", "USER");
  const [[max_n]] = await conn.query(
    `select max(order_id) as max_n from Orders`
  );
